...
from ohc_experience.sandbox.models import ProductWorkspace
from ohc_experience.sandbox.models import TicketAttachment
from ohc_experience.sandbox.models import TicketContext
...
class Command(BaseCommand):
    help = "Create a fresh, synthetic ABDM sandbox dataset. --reset deletes local data."
    ...
    def handle(self, *args, **options):  # noqa: PLR0915
        ...
        if not workspace:
            raise CommandError(str(form.errors))
        registration = workspace.product.sandbox_reviews.get(
        ...
        draft = workspace.product.milestones.get(key="uhi1").application.sandbox_review
    ...
    def exit(self, workspace, key, applicant, admin, reviewer, state):  # noqa: PLR0913, PLR0917
        item = workspace.product.milestones.get(key=key).application.sandbox_review
