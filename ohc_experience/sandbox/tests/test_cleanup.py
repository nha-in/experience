...
def test_only_current_applications_are_registered():
    assert {definition.key for definition in registry.all()} == {
        "abdm_sandbox_product",
        "abdm_sandbox_exit",
    }


def test_dashboard_uses_current_portal():
    assert resolve(reverse("dashboard")).func is resolve(reverse("sandbox:home")).func


def test_navigation_between_portal_and_settings_uses_full_pages(
    client, owner_membership,
):
    client.force_login(owner_membership.user)
    response = client.get(reverse("sandbox:products"))
    assert response.status_code == HTTPStatus.OK
    assert (
        f'href="{reverse("organisations:team")}" hx-boost="false"'
        in response.text
    )
    response = client.get(reverse("organisations:team"))
    assert response.status_code == HTTPStatus.OK
    assert (
        f'href="{reverse("sandbox:organisation")}" hx-boost="false"'
        in response.text
    )
