...
from ohc_experience.sandbox import credentials
from ohc_experience.sandbox import services
from ohc_experience.sandbox import uploads
from ohc_experience.sandbox.catalog import TRACK_MAP
from ohc_experience.sandbox.forms import ExitEvidenceForm
from ohc_experience.sandbox.forms import OrganisationForm
from ohc_experience.sandbox.forms import ProductRegistrationForm
from ohc_experience.sandbox.management.commands.seed_sandbox_demo import evidence_data
from ohc_experience.sandbox.management.commands.seed_sandbox_demo import (
    organisation_data,
)
from ohc_experience.sandbox.management.commands.seed_sandbox_demo import product_data
from ohc_experience.sandbox.models import AuditEvent
from ohc_experience.sandbox.models import Notification
from ohc_experience.sandbox.models import SandboxCredential
...
def environment(settings, tmp_path):
    ...
    registration = workspace.product.sandbox_reviews.get(kind="product_registration")
...
def milestone(environment, key="m1"):
    return (
        ...
        .product.milestones.get(key=key)
        .application.sandbox_review
    )
...
def test_approved_track_selection_cannot_be_removed(environment):
    approve(environment)
    registration = environment["workspace"].product.sandbox_reviews.get(
...
def test_credentials_encrypted_audited_rate_limited_and_not_in_outcomes(
    ...
    credential = SandboxCredential.objects.get(product=environment["workspace"].product)
    ...
    url = reverse("sandbox:credentials", args=[environment["workspace"].sandbox_id])
...
def test_callback_rejects_private_dns_results(address):
    with (
        patch(
            "ohc_experience.sandbox.credentials.socket.getaddrinfo",
...
def test_permission_boundaries_and_all_portal_pages_render(environment, client):
    ...
    client.force_login(environment["applicant"])
    paths = [
        reverse("sandbox:products"),
        workspace.get_absolute_url(),
        reverse("sandbox:organisation"),
        reverse("sandbox:product-edit", args=[workspace.sandbox_id]),
        reverse("sandbox:pending-queries"),
        reverse("sandbox:events"),
        reverse("sandbox:support"),
        reverse("sandbox:submission", args=[item.pk, item.selected_submission_id]),
    ]
    paths += [
        reverse("sandbox:track", args=[workspace.sandbox_id, code])
    ...
    assert client.get(reverse("sandbox:queue")).status_code == 403
    ...
    assert client.get(reverse("sandbox:attachment", args=[file.pk])).status_code == 404
    client.force_login(environment["reviewer"])
    for path in [
        reverse("sandbox:assess-dashboard"),
        reverse("sandbox:queue"),
        item.get_absolute_url(),
        item.get_absolute_url() + "?action=query&field=wasa_agency",
    ]:
        response = client.get(path)
        assert response.status_code == 200, path
    assert client.get(reverse("sandbox:attachment", args=[file.pk])).status_code == 200
...
def test_historical_schema_is_rendered_after_form_definition_changes(
    ...
    snapshot.save()
    client.force_login(environment["applicant"])
    response = client.get(reverse("sandbox:submission", args=[item.pk, snapshot.pk]))
...
def test_sent_back_draft_retains_reason_and_decision_history(environment, client):
    ...
    services.assign_review(item, environment["admin"], environment["reviewer"])
    services.decide(
        item, environment["reviewer"], action="send_back", note="Revise scope.",
    )
    item, form, saved = services.save_review_form(
        item,
        environment["applicant"],
    ...
    assert item.status == "in_review"
    client.force_login(environment["reviewer"])
    response = client.get(reverse("sandbox:assess-dashboard"))
...
def test_support_members_cannot_reply_or_withdraw(environment, client):
    ...
    Membership.objects.create(
        user=supporter, organisation=environment["org"], role="support",
    )
    item = submit(environment)
    services.assign_review(item, environment["admin"], environment["reviewer"])
    services.decide(
        item, environment["reviewer"], action="query", note="Clarify scope.",
    )
    client.force_login(supporter)
    response = client.get(
        reverse("sandbox:track", args=[environment["workspace"].sandbox_id, "HI-CM"]),
    )
    assert response.status_code == 200
...
def test_review_mutation_requires_csrf(environment):
    ...
    client.force_login(environment["applicant"])
    assert client.post(reverse("sandbox:withdraw", args=[item.pk])).status_code == 403
    item.refresh_from_db()
    assert item.pending
