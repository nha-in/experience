(() => {
  function updateExit(form) {
    const button = form.querySelector('[data-request-exit]');
    const reason = form.querySelector('[data-exit-reason]');
    if (!button) return;
    ...
    if (reason) reason.textContent = blocked ? 'Organisation and product approval required.' : button.disabled ? 'Complete all required fields and both testing documents.' : '';
  }
  ...
  function initialize(scope = document) {
    scope.querySelectorAll?.('[data-exit-form]').forEach(updateExit);
  ...
  document.addEventListener('change', event => {
    ...
    const form = event.target.closest('[data-exit-form]');
    if (form) updateExit(form);
  });
  ...
  document.addEventListener('drop', event => {
    const form = event.target.closest('[data-exit-form]');
    if (form) setTimeout(() => updateExit(form), 0);
  });
